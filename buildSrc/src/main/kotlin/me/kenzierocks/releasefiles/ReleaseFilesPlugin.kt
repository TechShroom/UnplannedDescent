...
class ReleaseFilesPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        val releasesBase = project.file("build/$releasesFolderName")

        // apps are a directory
        val appDir = releasesBase.toPath().resolve("${project.name}.app").resolve("Contents")
        val appResources = appDir.resolve("Resources")
        val appMacOs = appDir.resolve("MacOS")

        val srcResources = project.file("release-resources").toPath()

        // NOTE: paths are broken with gradle -- don"t ever pass them to gradle

        val macReleaseApp = project.tasks.register("macReleaseApp") {
            ...
            doFirst {
                ...
                val resourceRoot = appResources.resolve(project.name)
                project.copy {
                    from(project.zipTree(distZipTask.archiveFile))
                    into(resourceRoot.toFile())
                }
                // after copying zip content we must move it again
                // see https://github.com/gradle/gradle/issues/1108
                val baseDir = resourceRoot.resolve("${project.name}-${project.version}")
                ...
                infoPlist["CFBundleGetInfoString"] = NSString("${project.version}")
        ...
        val createExe = project.tasks.getByName("createExe")
        val shadowJar = project.tasks.getByName("shadowJar")
        createExe.dependsOn(shadowJar)

        project.configure<Launch4jPluginExtension> {
            mainClassName = project.the<ApplicationPluginConvention>().mainClassName
        ...
        val windowsReleaseExe = project.tasks.register("windowsReleaseExe") {
        ...
        val universalReleaseJar = project.tasks.register<Copy>("universalReleaseJar") {
            dependsOn(shadowJar)
            val shadowJarFile = shadowJar.outputs.files.first()
            val outFile = File(releasesBase, "${project.name}-${project.version}-universal.jar")
        ...
        val deleteExtraL4jJunk = project.tasks.register("deleteExtraLaunch4jJunk") {
        ...
        val macReleaseZip = project.tasks.register<Zip>("macReleaseZip") {
            dependsOn(macReleaseApp)
            from(macReleaseApp.get().outputs.files)

            archiveFileName.set("${project.name}-${project.version}-macOS.zip")
            destinationDirectory.set(releasesBase)
        }

        val osBundles = project.tasks.register("osBundles") {
        ...
        val deployOsBundles = project.tasks.register("deployOsBundles") {
            dependsOn(osBundles)
            ...
            doLast {
                ...
                val rel = repository
                        .createRelease(project.version.toString())
